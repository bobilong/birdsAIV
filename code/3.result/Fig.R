...
cattle2015_log <- log(cattle2015);plot(cattle2015_log)
cattle2015_log[cattle2015_log < -4] <- NA;plot(cattle2015_log)
#writeRaster(cattle2015_log, "/root/autodl-tmp/zyresult/Mantel_Data/cattle2015_log.tif")
# cattle_log<-rast("/root/autodl-tmp/zyresult/Mantel_Data/cattle2015_log.tif") %>% resample(globalRaster)%>% mask(globalCountry) %>% 
#   writeRaster('/root/autodl-tmp/humPoulResult/data/Mantel_data/corData/cattle_log.tif')
# 
#——————————————————————————————————————————————————————————-----------------
basePath<-"/root/autodl-tmp/humPoulResult/data/"
...
#fwrite(All5_poppoulcatresults_df,'/root/autodl-tmp/humPoulResult/data/Mantel_data/All5_poppoulcatresults_df.csv')




######饼图，最重要的------------
All5_poppoulcatresults_df<-fread('/root/autodl-tmp/humPoulResult/data/Mantel_data/All5_poppoulcatresults_df.csv')
...
lineardata_max
#fwrite(lineardata_max,"/root/autodl-tmp/humPoulResult/data/Mantel_data/lineardata_max_cattle.csv")
######——制作图例---------
...
color_China<-c("pop_All"="#6E0D20","pop_CH"="#B42D34","poul_All"="#6E0D20","poul_CH"="#B42D34","cattle_All"="#FAE3D7","cattle_CH"="#FAE3D7")
...
piedata_India$Value<-c(1,1,1,1,1,1)
color_India <- c("pop_All"="#6E0D20","pop_CH"="#D9705A","poul_All"="#FAE3D7","poul_CH"="#FAE3D7","cattle_All"="#B42D34","cattle_CH"="#D9705A")
...
p <- ggplot(piedata_India, aes(x = "", y = Value, fill = Order)) +
  geom_bar(stat = "identity", width = 1) +
  coord_polar("y", start = 0) +
  scale_fill_manual(values = color_India) +
  theme_void() +
  theme(
    panel.background = element_rect(fill = "transparent"), # 透明的面板背景
    plot.background = element_rect(fill = "transparent", color = NA), # 透明的绘图背景
...
piedata_EU$Value<-c(1,1,1,1,1,1)
color_EU<-c("pop_All"="#F2B396","pop_CH"="#F2B396","poul_All"="#F2B396","poul_CH"="#F2B396","cattle_All"="#F2B396","cattle_CH"="#F2B396")
...
piedata_Nigeria$Value<-c(1,1,1,1,1,1)

color_Nigeria<-c("pop_All"="#FAE3D7","pop_CH"="#FAE3D7","poul_All"="#FAE3D7","poul_CH"="#FAE3D7","cattle_All"="#B42D34","cattle_CH"="#6E0D20")
...
color_US<-c("pop_All"="#B42D34","pop_CH"="#D9705A","poul_All"="#B42D34","poul_CH"="#D9705A","cattle_All"="#FAE3D7","cattle_CH"="#FAE3D7")
...
x2<-factor(US_birdEntropy_df_long$Variable,levels = c('Others','Large wading birds','Waterfowls','Shorebirds','Seabirds')) #自定义展示顺序
